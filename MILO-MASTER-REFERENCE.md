...
| FSRS spaced repetition | Adopted. Wire during P4 Firestore migration. Adds stability, difficulty, due, lastReview per word progress entry. |
| Output drills 2× XP | type-it, listen-type, fill-blank, sentence-builder, conjugation, gender all award 2× XP. Standard XP for all other drills. |
| contextSentence | Batch-generated by Claude via Google AI Studio. One simple A1-A2 Spanish sentence per word. |
| 5 new words/day cap | Soft nudge only after 5 new words in a session. Never a hard block. Milo advises, never restricts. |
| Stop completion gate | Soft nudge if no output drill attempted before Stop completion — "You haven't practiced these words yet — want to give Type It a go before moving on? 🐾" Never blocks. |
| Contextual Binding | Output drills only. contextSentence shown 2–3 seconds after correct answer before advancing. |
| Fetch word selection | Output-weak words only until Path 2 complete. Then blended 80/20 — 80% due words, 20% output-weak. |
| Mastery tier thresholds | New = no attempts / Learning = stability 0–3 days / Strong = 4–21 days / Mastered = 22+ days AND outputCorrect: true |
| Output-weak tracking | FSRS + outputCorrect boolean per word in Firestore progress entry. Mastery requires both. |
| Smart Scheduling (FSRS ratings) | Togglable in Settings, default off. Off = silent Good/Again mapping. On = Hard/Good/Easy continue buttons after correct answers. Wrong always auto-rates Again. |
