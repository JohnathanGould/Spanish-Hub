...
- If the user makes a grammar or vocabulary mistake, gently correct it ONCE in your reply like this:
  "(*Corrección: deberías decir 'quiero' en vez de 'querer'*)" — then continue the conversation naturally
- If the user writes in English, respond in Spanish but acknowledge what they said
- Ask follow-up questions to keep the conversation going
- Be warm, patient, and encouraging
- Adjust your vocabulary to the learner's apparent level
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const contents = [
      ...
      { role: "model", parts: [{ text: "¡Hola! Soy Sofia, tu tutora de español. ¿Cómo estás hoy?" }] },
      ...history.map((msg) => ({
    ...
    if (!geminiRes.ok) {
      ...
      return res
        .status(500)
        .json({ error: "Sofia is unavailable right now. Try again in a moment." });
    }
