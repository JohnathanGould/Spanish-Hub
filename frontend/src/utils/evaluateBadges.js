...
// All standalone drill types for drill_all_types badge
// 'shadow' and 'word-sort' included — badge will not fire until those drillIds
// appear in sessions[], which only happens once those drills are built and wired
...
// Tier definitions — pathIds that belong to each tier
...
export function evaluateBadges(prevData, nextData, eventType, eventPayload) {
  ...
  if (eventType === 'drill_complete') {
    ...
    // Volume — uses totalDrills counter, not sessions[] length
    ...
    // First completion of each drill type
    const drillFirstMap = {
    ...
    // Drill variety — all types seen in sessions[]
    const seenDrillIds = new Set(sessions.map(s => s.drillId));
    if (ALL_DRILL_TYPES.size > 0 && [...ALL_DRILL_TYPES].every(id => seenDrillIds.has(id)) && !has('drill_all_types')) {
    ...
    // Ghost — returned after 7+ day gap
  ...
  if (eventType === 'answer') {
    ...
    const xpThresholds = [
      [500, 'xp_500'], [2000, 'xp_2000'], [5000, 'xp_5000'],
      [12500, 'xp_12500'], [25000, 'xp_25000'], [50000, 'xp_50000'],
    ];
    ...
    const masteryThresholds = [
      [1, 'mastery_1'], [10, 'mastery_10'], [25, 'mastery_25'],
      [50, 'mastery_50'], [75, 'mastery_75'], [125, 'mastery_125'],
      [200, 'mastery_200'], [MASTER.length, 'mastery_309'],
    ];
    for (const [threshold, id] of masteryThresholds) {
      if (masteredCount >= threshold && !has(id)) earn(id);
    }

    // Bones milestones — uses totalBonesEarned lifetime counter
    const totalBones = nextData.totalBonesEarned || 0;
    const bonesThresholds = [
      [50, 'bones_50'], [100, 'bones_100'], [250, 'bones_250'], [500, 'bones_500'],
    ];
    for (const [threshold, id] of bonesThresholds) {
  ...
  if (eventType === 'stop_complete') {
    ...
    // First stop of each Path — pattern p{N}s1
    const firstStopMap = {
      'p1s1': 'stop_1_1', 'p2s1': 'stop_2_1', 'p3s1': 'stop_3_1',
      'p4s1': 'stop_4_1', 'p5s1': 'stop_5_1', 'p6s1': 'stop_6_1',
      'p7s1': 'stop_7_1', 'p8s1': 'stop_8_1', 'p9s1': 'stop_9_1',
      'p10s1': 'stop_10_1', 'p11s1': 'stop_11_1', 'p12s1': 'stop_12_1',
      'p13s1': 'stop_13_1',
    };
    // Midpoint stop of each Path — Stop 3
    const midStopMap = {
      'p1s3': 'stop_1_3', 'p2s3': 'stop_2_3', 'p3s3': 'stop_3_3',
      'p4s3': 'stop_4_3', 'p5s3': 'stop_5_3', 'p6s3': 'stop_6_3',
      'p7s3': 'stop_7_3', 'p8s3': 'stop_8_3', 'p9s3': 'stop_9_3',
      'p10s3': 'stop_10_3', 'p11s3': 'stop_11_3', 'p12s3': 'stop_12_3',
      'p13s3': 'stop_13_3',
    };
    // Final stop of each Path — Stop 5
    const finalStopMap = {
      'p1s5': 'stop_1_5', 'p2s5': 'stop_2_5', 'p3s5': 'stop_3_5',
      'p4s5': 'stop_4_5', 'p5s5': 'stop_5_5', 'p6s5': 'stop_6_5',
      'p7s5': 'stop_7_5', 'p8s5': 'stop_8_5', 'p9s5': 'stop_9_5',
      'p10s5': 'stop_10_5', 'p11s5': 'stop_11_5', 'p12s5': 'stop_12_5',
      'p13s5': 'stop_13_5',
    };
  ...
  if (eventType === 'path_complete') {
    ...
    const pathBadgeMap = {
      'path1': 'path_1', 'path2': 'path_2', 'path3': 'path_3',
      'path4': 'path_4', 'path5': 'path_5', 'path6': 'path_6',
      'path7': 'path_7', 'path8': 'path_8', 'path9': 'path_9',
      'path10': 'path_10', 'path11': 'path_11', 'path12': 'path_12',
      'path13': 'path_13',
    };
    ...
    // Tier completion
    if (BEGINNER_PATHS.every(p => completedPaths.includes(p)) && !has('tier_beginner')) {
      earn('tier_beginner');
    }
    if (ADVANCED_BEGINNER_PATHS.every(p => completedPaths.includes(p)) && !has('tier_advanced_beginner')) {
      earn('tier_advanced_beginner');
    }
    if (pathId === 'path13' && !has('tier_intermediate')) {
      earn('tier_intermediate');
    }
  }
